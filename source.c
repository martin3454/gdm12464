...
//funkce pro zapis pixelu do pole
...
//funkce pro mazani pixelu v poli
...
/*void GLCD_ClearScreen(){
	...
	PTC->PSOR = CS1;
	for(int i=0;i<8;i++){
		GLCD_Write_Cmd(0x40); //Y adres counter
		GLCD_Write_Cmd(0xb8 + i); //X page adres
...
void Draw_Circle(uint8_t x, uint8_t y, uint8_t r){
	...
	while (xx >= yy) {
		...
		GLCD_SetPixel(x + xx, y + yy);
		GLCD_SetPixel(x - xx, y + yy);
		GLCD_SetPixel(x - xx, y - yy);
		GLCD_SetPixel(x + xx, y - yy);
		GLCD_SetPixel(x + yy, y + xx);
		GLCD_SetPixel(x - yy, y + xx);
		GLCD_SetPixel(x - yy, y - xx);
		GLCD_SetPixel(x + yy, y - xx);
...
void Fill_Circle(uint8_t x1, uint8_t y1, uint8_t r){
	for(int y=-r; y<=r; y++){
		for(int x=-r; x<=r; x++){
			if(x*x + y*y <=r*r) GLCD_SetPixel(x1+x, y1+y);
		}
	}
